partial class plong {
    public static void update() {
        ...
        if (!ff) {
            ...
            if (ballvel.X < 0) {
                if (ballpos.Y > paddleLpos.Y - 9 && ballpos.Y < paddleLpos.Y + 9 && ballpos.X < paddleLpos.X + 2)
                { ballvel.X *= -1; ballvel.Y = (ballpos.Y-paddleLpos.Y)/8; hitOut.Stop(); hitPS(); }
            } else {
                if (ballpos.Y > paddleRpos.Y - 9 && ballpos.Y < paddleRpos.Y + 9 && ballpos.X > paddleRpos.X - 2)
